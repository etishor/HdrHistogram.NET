...
namespace HdrHistogram.Tests
{
    ...
    public class DoubleHistogramDataAccessTest
    {
        ...
        static DoubleHistogram scaledHistogram;
        static DoubleHistogram rawHistogram;
        static DoubleHistogram scaledRawHistogram;
        static DoubleHistogram postCorrectedHistogram;
        static DoubleHistogram postCorrectedScaledHistogram;

        static DoubleHistogramDataAccessTest()
        {
        ...
        private void assertEquals(string because, long expected, long value)
        {
        ...
        {
            assertEquals("averages should be equivalent",
                    histogram.getMean() * 512,
                    scaledHistogram.getMean(), scaledHistogram.getMean() * 0.000001);
            assertEquals("total count should be the same",
                    histogram.getTotalCount(),
                    scaledHistogram.getTotalCount());
            assertEquals("99%'iles should be equivalent",
            ...
            assertEquals("Max should be equivalent",
        ...
        public void testPreVsPostCorrectionValues()
        {
            ...
            assertEquals("pre and post corrected count totals ",
            ...
            //            assertEquals("pre and post corrected count at value " + v.getValueIteratedTo(),
        ...
        public void testGetTotalCount()
        {
            ...
            assertEquals("Raw total count is 10,001",
                    10001L, rawHistogram.getTotalCount());
            assertEquals("Total count is 20,000",
        ...
        public void testGetMean()
        {
            ...
            assertEquals("Raw mean is " + expectedRawMean + " +/- 0.1%",
                    expectedRawMean, rawHistogram.getMean(), expectedRawMean * 0.001);
            assertEquals("Mean is " + expectedMean + " +/- 0.1%",
        ...
        public void testGetStdDeviation()
        {
            ...
            assertEquals("Raw standard deviation is " + expectedRawStdDev + " +/- 0.1%",
                    expectedRawStdDev, rawHistogram.getStdDeviation(), expectedRawStdDev * 0.001);
            assertEquals("Standard deviation is " + expectedStdDev + " +/- 0.1%",
        ...
        {
            assertEquals("Raw percentile at or below 5 msec is 99.99% +/- 0.0001",
                    99.99,
                    rawHistogram.getPercentileAtOrBelowValue(5000), 0.0001);
            assertEquals("Percentile at or below 5 msec is 50% +/- 0.0001%",
                    50.0,
                    histogram.getPercentileAtOrBelowValue(5000), 0.0001);
            assertEquals("Percentile at or below 100 sec is 100% +/- 0.0001%",
                    100.0,
        ...
        {
            assertEquals("Count of raw values between 1 msec and 1 msec is 1",
                    10000, rawHistogram.getCountBetweenValues(1000L, 1000L), 10000 * 0.000001);
            assertEquals("Count of raw values between 5 msec and 150 sec is 1",
                    1, rawHistogram.getCountBetweenValues(5000L, 150000000L), 1 * 0.000001);
            assertEquals("Count of values between 5 msec and 150 sec is 10,000",
        ...
        {
            assertEquals("Count of raw values at 10 msec is 0",
                    0, rawHistogram.getCountBetweenValues(10000L, 10010L), 0.000001);
            assertEquals("Count of values at 10 msec is 0",
                    1, histogram.getCountBetweenValues(10000L, 10010L), 0.000001);
            assertEquals("Count of raw values at 1 msec is 10,000",
                    10000, rawHistogram.getCountAtValue(1000L), 10000 * 0.000001);
            assertEquals("Count of values at 1 msec is 10,000",
        ...
        {
            int i = 0;
            foreach (DoubleHistogramIterationValue v in histogram.percentiles(5 /* ticks per half */))
            {
                assertEquals("i = " + i + ", Value at Iterated-to Percentile is the same as the matching getValueAtPercentile():\n" +
        ...
        public void testLinearBucketValues()
        {
            ...
            foreach (DoubleHistogramIterationValue v in rawHistogram.linearBucketValues(100000))
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Raw Linear 100 msec bucket # 0 added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                else if (index == 999)
                {
                    assertEquals("Raw Linear 100 msec bucket # 999 added a count of 1",
                            1, countAddedInThisBucket);
                }
                else
                {
                    assertEquals("Raw Linear 100 msec bucket # " + index + " added a count of 0",
            ...
            foreach (DoubleHistogramIterationValue v in histogram.linearBucketValues(10000))
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Linear 1 sec bucket # 0 [" +
            ...
            assertEquals("There should be 10000 linear buckets of size 10000 usec between 0 and 100 sec.",
                    10000, index);
            assertEquals("Total added counts should be 20000", 20000, totalAddedCounts);

            index = 0;
            totalAddedCounts = 0;
            // Iterate data using linear buckets of 1 msec each.
            foreach (DoubleHistogramIterationValue v in histogram.linearBucketValues(1000))
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
            ...
            assertEquals("There should be 100007 linear buckets of size 1000 usec between 0 and 100 sec.",
                    100007, index);
            assertEquals("Total added counts should be 20000", 20000, totalAddedCounts);
        ...
        public void testLogarithmicBucketValues()
        {
            ...
            foreach (DoubleHistogramIterationValue v in rawHistogram.logarithmicBucketValues(10000, 2))
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Raw Logarithmic 10 msec bucket # 0 added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                else if (index == 14)
                {
                    assertEquals("Raw Logarithmic 10 msec bucket # 14 added a count of 1",
                            1, countAddedInThisBucket);
                }
                else
                {
                    assertEquals("Raw Logarithmic 100 msec bucket # " + index + " added a count of 0",
                            0, countAddedInThisBucket);
                }
                index++;
            }
            ...
            foreach (DoubleHistogramIterationValue v in histogram.logarithmicBucketValues(10000, 2))
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Logarithmic 10 msec bucket # 0 [" +
                            v.getValueIteratedFrom() + ".." + v.getValueIteratedTo() +
                            "] added a count of 10001",
                            10001, countAddedInThisBucket);
                }
            ...
            assertEquals("There should be 14 Logarithmic buckets of size 10000 usec between 0 and 100 sec.",
                    14, index - 1);
            assertEquals("Total added counts should be 20000", 20000, totalAddedCounts);
        }
        ...
        public void testRecordedValues()
        {
            ...
            foreach (DoubleHistogramIterationValue v in rawHistogram.recordedValues())
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Raw recorded value bucket # 0 added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                else
                {
                    assertEquals("Raw recorded value bucket # " + index + " added a count of 1",
                            1, countAddedInThisBucket);
                }
                index++;
            }
            ...
            long totalAddedCounts = 0;
            // Iterate data using linear buckets of 1 sec each.
            foreach (DoubleHistogramIterationValue v in histogram.recordedValues())
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 0)
                {
                    assertEquals("Recorded bucket # 0 [" +
                            v.getValueIteratedFrom() + ".." + v.getValueIteratedTo() +
                            "] added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                Assert.True(v.getCountAtValueIteratedTo() != 0, "The count in recorded bucket #" + index + " is not 0");
                assertEquals("The count in recorded bucket #" + index +
        ...
        public void testAllValues()
        {
            ...
            foreach (DoubleHistogramIterationValue v in rawHistogram.allValues())
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 2000)
                {
                    assertEquals("Raw allValues bucket # 0 added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                else if (histogram.valuesAreEquivalent(v.getValueIteratedTo(), 100000000))
                {
                    assertEquals("Raw allValues value bucket # " + index + " added a count of 1",
                            1, countAddedInThisBucket);
                }
                else
                {
                    assertEquals("Raw allValues value bucket # " + index + " added a count of 0",
                            0, countAddedInThisBucket);
                }
                latestValueAtIndex = v.getValueIteratedTo();
                totalCountToThisPoint += v.getCountAtValueIteratedTo();
                assertEquals("total Count should match", totalCountToThisPoint, v.getTotalCountToThisValue(), 1e-8);
                totalValueToThisPoint += v.getCountAtValueIteratedTo() * latestValueAtIndex;
                assertEquals("total Value should match", totalValueToThisPoint, v.getTotalValueToThisValue(), 1e-8);
                index++;
            }
            assertEquals("index should be equal to countsArrayLength",
            ...
            long totalAddedCounts = 0;
            // Iterate data using linear buckets of 1 sec each.
            foreach (DoubleHistogramIterationValue v in histogram.allValues())
            {
                long countAddedInThisBucket = v.getCountAddedInThisIterationStep();
                if (index == 2000)
                {
                    assertEquals("AllValues bucket # 0 [" +
                            v.getValueIteratedFrom() + ".." + v.getValueIteratedTo() +
                            "] added a count of 10000",
                            10000, countAddedInThisBucket);
                }
                assertEquals("The count in AllValues bucket #" + index +
                        " is exactly the amount added since the last iteration ",
                        v.getCountAtValueIteratedTo(), v.getCountAddedInThisIterationStep());
                totalAddedCounts += v.getCountAddedInThisIterationStep();
                index++;
